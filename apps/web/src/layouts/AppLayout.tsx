...
const Topbar = ({
  ...
  <header className="flex items-center gap-3 border-b border-[color:var(--line)] bg-[color:var(--bg)] px-4 py-3 md:px-5">
    ...
    <button
      type="button"
      onClick={onSearch}
      className="r-mobile-hide hidden items-center gap-2.5 rounded-md border border-[color:var(--line)] bg-[color:var(--bg-1)] px-3 py-1.5 text-left md:flex md:min-w-[260px]"
    ...
    <Link to="/notifications" className="btn sm r-mobile-hide" aria-label="Notifications">
      <Icons.bell />
    </Link>
    ...
    <>
      ...
      {subscription.isReadOnly && (
        <div
          role="status"
          aria-live="polite"
          className="bg-[color:var(--color-danger)]/10 px-4 py-2 text-center text-[13px] text-[color:var(--color-danger)]"
        >
